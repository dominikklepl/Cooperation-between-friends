...
```{r}
Q1 = read.csv("data/words_rating/Questions_1.csv")
Q2 = read.csv("data/words_rating/Questions_2.csv")
Q3 = read.csv("data/words_rating/Questions_3.csv")
Q4 = read.csv("data/words_rating/Questions_4.csv")
Q5 = read.csv("data/words_rating/Questions_5.csv")
```
...
apply(Q2,1,function(x) length(unique(x))) #2=5, 7=1
apply(Q3,1,function(x) length(unique(x)))
apply(Q4,1,function(x) length(unique(x))) #1=2
apply(Q5,1,function(x) length(unique(x))) #3=2, 9=3
...
links_rated = links_rated %>% group_by(link) %>% summarise(semantics_mean=mean(rating),semantics_median=median(rating),semantics_max=max(rating),semantics_min=min(rating))
