...
plot(m_confidence_round_random)
pairs(m_confidence_round_random)

compare(m_confidence,m_confidence_random,m_confidence_success,m_confidence_success_random)

plot(precis(m_confidence_success_random)) #we've got effect AND interaction
```

# 2.4 Hypothesis 4: Semantics
```{r}
m_semantic = map2stan(
    alist(
        friends ~ dbinom( 1 , p ) ,
...
compare(m_semantic,m_semantic_random,m_semantic_success,m_semantic_success_random)

plot(precis(m_semantic_success_random)) #and again, 1 effect and interaction
```


## 2.5 Combinations of models and full model
```{r}
m_fixed= map2stan(
    alist(
        friends ~ dbinom( 1 , p ) ,
        logit(p) <- a[ID] + bP*performance +  bL*linked_n + bC*confidence + bSE*semantic,
        a[ID] ~ dnorm(0,0.05),
        bP ~ dnorm(0,0.2),
        bL ~ dnorm(0,0.2),
        bC ~ dnorm(0,0.2),
...
m_full = map2stan(
    alist(
        friends ~ dbinom( 1 , p ) ,
        logit(p) <- a[ID] + bP*performance + bC*confidence + bSE*semantic + bCS*confidence*is_success + bSES*semantic*is_success,
        a[ID] ~ dnorm(0,0.05),
        bP ~ dnorm(0,0.2),
        bC ~ dnorm(0,0.2),
        bSE ~ dnorm(0,0.2),
...
```{r}
comparison=compare(m_performance_random,m_confidence_success_random,m_semantic_success_random,m_fixed,m_full)
...
y_performance = sim(m_performance_round_random,n=1e4)
y_confidence = sim(m_confidence_success_random,n=1e4)
y_semantic = sim(m_semantic_success_random,n=1e4)
y_fixed = sim(m_fixed,n=1e4)
y_full = sim(m_full,n=1e4)

# + an ensemble model
ens = ensemble(m_performance_round_random,m_confidence_success_random,m_semantic_success_random,m_fixed,m_full)

y_ens = ens$sim

ppc_stat(y,y_performance_round,stat = 'success',binwidth = 0.01)
ppc_stat(y,y_fixed,stat = 'success',binwidth = 0.01)
...
```{r}
#performance and linked_n effect - m_fixed
(results_fixed = precis(m_fixed))
plot(results_fixed)
results_fixed = round(results_fixed@output,3)
write.csv(results_fixed,"Results/Long/Results_fixed.csv",row.names = F)
...
performance_vis = data.frame(
  ...
  confidence = rep(0,length(perf)),
  semantic = rep(0,length(perf)),
  friends = rep(0,length(perf)),
  ID= rep(sample(1:10,1),length(perf))
)

#compute the mean prediction + 89% intervals
perf_l = link(m_fixed,data=performance_vis,n=1e4)
...
(performance_plot=ggplot(performance_vis,aes(x=performance))+
  ...
  labs(title="Effect of performace",x="performance in %",y="Odds"))
...
linked_n_vis = data.frame(
  ...
  confidence = rep(0,length(linked)),
  semantic = rep(0,length(linked)),
  friends = rep(0,length(linked)),
  ID= rep(sample(1:10,1),length(linked))
)

#compute the mean prediction + 89% intervals
linked_l = link(m_fixed,data=linked_n_vis,n=1e4)
...
(linked_n_plot=ggplot(linked_n_vis,aes(x=linked_n))+
  geom_line(aes(y=mean))+
  geom_ribbon(aes(ymin=`5%`,ymax=`94%`),alpha=0.5,fill="#0092ff")+
  labs(title="Effect of linked_n",x="n of linked words",y="Odds"))

ggsave("Figures/Long/Linked_n_visualization.jpg",linked_n_plot)
```

And now interaction of confidence with is_success
```{r}
...
(semantic_plot=ggplot(semantic_vis,aes(x=semantic))+
  ...
  geom_ribbon(aes(ymin=`5%`,ymax=`94%`),alpha=0.5,fill="#0092ff")+
  facet_grid(~is_success)+
  labs(title="Interaction of usage of common ground with successfulness of the round",y="Odds",x="Usage of common ground"))
