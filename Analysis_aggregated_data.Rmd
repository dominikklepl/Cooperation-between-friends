...
#look at the results
(mistakes_results = precis(m_mistakes,depth = 1))
...
(odds_plot_mistakes=ggplot(mistakes_vis,aes(x=mistakes))+
  ...
  labs(title="Effect of mistakes",x="Odds",y="Number of mistakes"))

ggsave("Figures/Aggregated/Mistakes_visualization.jpg",odds_plot_mistakes)
```
...
```{r}
(all_result = precis(m_all,depth = 1))
all_result=round(all_result@output,3)

plot(precis(m_all_random,depth = 1))

write.csv(all_result,"Results/Aggregated/model_all.csv")
```
...
all_l_rounds=link(m_all,data=all_vis_rounds,1e4)
...
(odds_plot_rounds_all=ggplot(all_vis_rounds,aes(x=n_rounds))+
  ...
  labs(title="Effect of rounds",x="Number of rounds",y="Odds"))
...
```{r}
samples_mistakes = extract.samples(m_mistakes,n=1e4)
samples_all = extract.samples(m_all,n=1e4)
